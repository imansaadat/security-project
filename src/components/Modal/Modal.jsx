     ...
     {encrptedData || decryptedData ? (
       <div className="p-6">
            ...
            <p className="text-white break-words leading-7">{screen === "encrypt" ? encrptedData : decryptedData}</p>
             ...
             {showCopyMessage && <p className="flex items-center justify-center gap-2 text-2xl text-green-400 text-center mt-5 capitalize">{showCopyMessage} <FaSmile className="relative" size={25}/></p>}
          </div>
        ) : null
        }
     </div>
